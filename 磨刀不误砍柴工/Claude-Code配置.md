...
![](assets/Claude-Code配置/file-20260601204525994.png)

**步骤2：**安装完成后，**务必在打开Claude Code之前**，优先设置CC Switch。在CC Switch的Claude Code页面添加API Key供应商：![](assets/Claude-Code配置/file-20260530193421977.png)

**步骤3：**以 **DeepSeek** 为例，选择对应厂商（中国版），然后填写API Key并配置模型：
![](assets/Claude-Code配置/file-20260530193421974.png)
可进行`模型测试`，会提示**运行正常**
![697](assets/Claude-Code配置/file-20260530193421970.png)
可进行`配置用量查询`,勾选后即可查看余额
![](assets/Claude-Code配置/file-20260530193421968.png)
...
![](assets/Claude-Code配置/file-20260530193421966.png)
...
2. CC在Accept Edits模式下，执行终端命令时，下图三个选项的含义依次分别是：
    ...
    **不同意**，再商量
    ...
    ![](assets/Claude-Code配置/file-20260530193421964.png)
...
- 给CC提供 [CC-StatusLine配置](CC-StatusLine配置.md) ，并对CC输入以下提示词，根据CC引导完成后重启终端即可：
  
```
帮我配一个 Claude Code statusLine，显示当前目录 + 模型名 + 上下文使用百分比。以后每次打开 claude（不管什么项目）都能自动显示。

用 cc-statusline-cli（npm 包）方案，配置自定义显示：directory + model + context_bar 一行，中文 label "使用"。
```
...
进一步参考 [CC-skills-MCP配置](CC-skills-MCP配置.md) 的内容
