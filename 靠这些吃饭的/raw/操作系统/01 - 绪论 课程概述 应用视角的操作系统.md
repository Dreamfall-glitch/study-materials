...
`while (1) {     stmt = fetch_statement();     decode_and_execute(stmt); }`

- (我们让 LLM 改写了 printf，调试一下试试吧！)

# [C 解释执行器](https://jyywiki.cn/OS/demos/intro/cinterp)
...
![center w:600](https://jyywiki.cn/OS/2026/static/img/tower-of-hanoi.jpg)

# 理解函数调用：终极考题

`int f(int n) { return (n<=1) ? 1 : f(n-1) + g(n-2); } int g(int n) { return (n<=1) ? 1 : f(n+1) + g(n-1); }`
...
# [汉诺塔](https://jyywiki.cn/OS/demos/intro/hanoi-nr)

汉诺塔是递归和分治的经典问题，而同学们也曾经在理解这个程序的时候遇到困难。遇到困难是正常的：C/C++ 中的 “函数” 和数学的函数很不一样，例如我们可以把 Fibonacci 数列的递归写成

`int x = f(n - 1); int y = f(n - 2); // 也可以 return y + x; return x + y;`
...
# [RISC-V 处理器模拟器](https://jyywiki.cn/OS/demos/intro/mini-rv32ima)
...
# [什么是编译器](https://jyywiki.cn/OS/demos/intro/ccompile)
...
# [最小可执行文件](https://jyywiki.cn/OS/demos/intro/minimal)
...
![](https://jyywiki.cn/OS/2026/static/img/minix3-desktop.png)
...
![](https://jyywiki.cn/OS/2026/static/img/rich4.jpg)
